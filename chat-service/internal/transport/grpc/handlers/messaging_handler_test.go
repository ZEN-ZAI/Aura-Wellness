...
type mockMessagingSvc struct {
	SendMessageFn    func(ctx context.Context, workspaceID, conversationID, personID uuid.UUID, senderName, content string) (entities.ChatMessage, error)
	ListMessagesFn   func(ctx context.Context, conversationID uuid.UUID, before time.Time, limit int) ([]entities.ChatMessage, error)
	StreamMessagesFn func(ctx context.Context, conversationID uuid.UUID) (<-chan entities.ChatMessage, func(), error)
}

func (m *mockMessagingSvc) SendMessage(ctx context.Context, wsID, convID, pID uuid.UUID, senderName, content string) (entities.ChatMessage, error) {
	return m.SendMessageFn(ctx, wsID, convID, pID, senderName, content)
}
func (m *mockMessagingSvc) ListMessages(ctx context.Context, convID uuid.UUID, before time.Time, limit int) ([]entities.ChatMessage, error) {
	return m.ListMessagesFn(ctx, convID, before, limit)
}
func (m *mockMessagingSvc) StreamMessages(ctx context.Context, convID uuid.UUID) (<-chan entities.ChatMessage, func(), error) {
	return m.StreamMessagesFn(ctx, convID)
}
...
func TestSendMessage_InvalidConversationUUID_ReturnsInvalidArgument(t *testing.T) {
	h := handlers.NewMessagingHandler(&mockMessagingSvc{})
	_, err := h.SendMessage(context.Background(), &pb.SendMessageRequest{
...
func TestSendMessage_AccessDenied_ReturnsPermissionDenied(t *testing.T) {
	h := handlers.NewMessagingHandler(&mockMessagingSvc{
		SendMessageFn: func(_ context.Context, _, _, _ uuid.UUID, _, _ string) (entities.ChatMessage, error) {
...
func TestListMessages_InvalidConversationUUID_ReturnsInvalidArgument(t *testing.T) {
	h := handlers.NewMessagingHandler(&mockMessagingSvc{})
